...
object List {
  ...
  // Exercise 3.14 Implement append with foldRight
  // Pretty straightforward using cons as the folding operation have the B param be the value we want to append
  def append_mine[A](as: List[A], a: A): List[A] =
    foldRight(as, List(a))((a, acc) => Cons(a, acc))

  // THE OFFICIAL SOLUTION
  // What I did wrong:
  //  - I misunderstood 'append' thinking it meant a single item to append as opposed to another list but the solution I have is the same principle
  //    I just wrap the value in a List instead
